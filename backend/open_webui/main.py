...
def send_to_feishu(message: str):
    if not ENABLE_FEISHU_NOTIFICATIONS:
        return
    requests.post(FEISHU_WEBHOOK, json={"msg_type": "text", "content": {"text": f"🚨 小智3.0异常通知：\n{message}"}})
...
app.state.config.ENABLE_CLAUDE_CODE = ENABLE_CLAUDE_CODE
app.state.config.CLAUDE_CODE_PATH = CLAUDE_CODE_PATH
app.state.config.CLAUDE_CODE_WORKSPACE_ROOT = CLAUDE_CODE_WORKSPACE_ROOT
app.state.config.CLAUDE_CODE_DANGEROUSLY_SKIP_PERMISSIONS = (
    CLAUDE_CODE_DANGEROUSLY_SKIP_PERMISSIONS
)
app.state.config.CLAUDE_CODE_DEFAULT_SYSTEM_PROMPT = CLAUDE_CODE_DEFAULT_SYSTEM_PROMPT
